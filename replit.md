...
-   **WhatsApp Integration**: Embedded functionality via `@whiskeysockets/baileys`. Each user has their own independent WhatsApp connection and session. Features include QR code connection, full chat page with RTL support, rich media support, message delivery statuses, and emoji picker. **Security**: Session-based data isolation ensures each user only sees their own WhatsApp chats and messages. **Data Integrity**: Text sanitization removes invalid UTF-8 characters (unpaired surrogates, null bytes) while preserving valid emojis and multilingual content (Hebrew, Arabic, etc.) to prevent PostgreSQL encoding errors.
